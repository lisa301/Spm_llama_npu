...
impl History {
    ...
    fn encode_message(message: &Message) -> Result<String> {
        let content = message.content.to_text_strict()?;
        Ok(Self::encode_header(message) + content.trim() + "<|eot_id|>")
    }
    ...
    pub fn encode_dialog_to_prompt(&self) -> Result<String> {
